...
{
    Gdiplus::GdiplusStartupInput gsi; Gdiplus::GdiplusStartup(&m_gdiplusToken, &gsi, NULL);
...
void CFactoryVisionClientView::OnDraw(CDC* pDC)
{
    ...
    Gdiplus::RectF rVideo(0, 0, (REAL)rc.Width() * 0.75f, (REAL)rc.Height());
    Gdiplus::RectF rDash(rVideo.Width, 0, (REAL)rc.Width() * 0.25f, (REAL)rc.Height());
...
void CFactoryVisionClientView::DrawVideo(Gdiplus::Graphics& g, const Gdiplus::RectF& r)
{
    ...
    if (!frame.empty()) DrawBitmap(g, frame, r, true);
        ...
        DrawTextWithShadow(g, _T("NO SIGNAL"), m_pFontLarge, r, &t, &f);
    }
}
...
void CFactoryVisionClientView::DrawDashboard(Gdiplus::Graphics& g, const Gdiplus::RectF& r)
{
    ...
    REAL titleH = 40.0f, summaryH = 60.0f;
    ...
    DrawTextWithShadow(g, _T("실시간 검사 결과"), m_pFontMedium, Gdiplus::RectF(inner.X, inner.Y, inner.Width, titleH), &white, &titleFmt);
    ...
    if (has)
    {
        ...
        DrawTextWithShadow(g, res.sFinalResult, m_pFontLarge, rRes, &resBrush, &center);

        CString info; info.Format(_T("품목: %s   시각: %s"), res.sProduct.GetString(), res.sTime.GetString());
        DrawTextWithShadow(g, info, m_pFontSmall, Gdiplus::RectF(inner.X, rRes.GetBottom() + 2, inner.Width, 22), &white, &center);
        if (!res.sDefectSummary.IsEmpty())
            DrawTextWithShadow(g, _T("요약: ") + res.sDefectSummary, m_pFontSmall, Gdiplus::RectF(inner.X, rRes.GetBottom() + 25, inner.Width, 22), &white, &center);
        ...
        for (const auto& d : res.vecDefects)
        {
            ...
            CString line; line.Format(_T("[%s] %s: %s (%.0f%%) %s"),
                ...
                d.fConfidence * 100.0f, d.sAdditionalInfo.IsEmpty() ? _T("") : _T("- ") + d.sAdditionalInfo);
            DrawTextWithShadow(g, line, m_pFontDefect, Gdiplus::RectF(rList.X, y, rList.Width, lh),
    ...
    else
    {
        ...
        DrawTextWithShadow(g, _T("SYSTEM NORMAL"), m_pFontLarge, Gdiplus::RectF(inner.X, inner.Y + titleH, inner.Width, inner.Height - titleH), &green, &c);
    }
}
...
void CFactoryVisionClientView::DrawBitmap(Gdiplus::Graphics& g, const cv::Mat& img, const Gdiplus::RectF& r, bool keepAR)
{
    ...
    Gdiplus::Bitmap bmp(img.cols, img.rows, img.step, PixelFormat24bppRGB, (BYTE*)img.data);
    Gdiplus::RectF rd = r;
    if (keepAR) {
        REAL rc = r.Width / r.Height, ri = (REAL)img.cols / (REAL)img.rows;
...
// 결과 수신: 불량 시 알람 + 팝업 + 히스토리 적재 (클라 MySQL 로깅은 제거됨)
...
bool CFactoryVisionClientView::ExportHistoryToCsv(const CString& filePath)
{
    ...
    CSingleLock lk(&m_csResult, TRUE);
    for (const auto& d : m_DefectHistory)
    {
        ...
        {
            for (const auto& it : r.vecDefects)
            {
                fwprintf(fp, L"%s,%s,%s,%s,%s,%s,%s,%.3f,%s\n",
                    r.sProduct.GetString(), r.sFinalResult.GetString(),
                    r.sTime.GetString(), r.sDefectSummary.GetString(),
                    it.sCameraType.GetString(), it.sDefectType.GetString(),
                    it.sDefectValue.GetString(), it.fConfidence,
                    it.sAdditionalInfo.GetString());
            }
        }
    }
    fclose(fp);
...
// JPG 저장 (./images/yyyymmdd_HHMMSS_ms_camX.jpg) — View측 저장도 동일 품질 사용
bool CFactoryVisionClientView::SaveJpegToFile(const cv::Mat& img, CString& outPath)
{
    ...
    std::vector<uchar> jpg; cv::imencode(".jpg", img, jpg, { cv::IMWRITE_JPEG_QUALITY, q });
